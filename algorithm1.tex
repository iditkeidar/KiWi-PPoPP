...
In Section~\ref{sec:organization}  we explain how data is organized in KiWi.   Section~\ref{sec:ops} discusses how the different operations are implemented atop this data structure in the common case, when it does not require maintenance.
 In Section~\ref{sec:rebalance} we discuss rebalancing.
...
Rebalancing not only splits (over-utilized) and merges (under-utilized) chunks as in a B+Tree, it also improves chunks' internal
...
KiWi employs two bookkeeping data structures for coordinating different operations:
...
\paragraph{Helping put operations.}
...
Because scans use version numbers in order to decide which puts they take into account, the  order of put operations is determined by the order of their versions.
...
This is illustrated in Figure~\ref{fig:linearization}, where the get must help \code{put(k2,b)} obtain a version, because ignoring it
...
\label{l:put-prep}
		\If{\codeF{checkRebalance($C$, \codeF{key, val}})}
			\State  \Comment rebalance has completed the put  \label{l:put-rebalance0}
			\State return
		\EndIf
		...
		\State \codeF{k[i]} $\leftarrow \langle\codeF{key}, \bot, \codeF{j}, \bot\rangle$ \Comment version not set yet
...
\Statex
	...
	\Procedure{helpPendingPuts}{fromKey, toKey, $C$}  \label{l:help-pending}
			\ForAll {$c$ in $C$.\codeF{ppa}} 				
				\State \codeF{idx} $\leftarrow$ $c$.\codeF{idx} 
				\If{ $C$.\codeF{k[idx].key} $\in$ [\codeF{fromKey, toKey}]} 
					\State \codeF{gv} $\leftarrow$ \codeF{GV}  \label{l:get-readGV}
					\State CAS($c$, $\langle \bot$, \codeF{idx} $\rangle$, $\langle$ \codeF{gv, idx} $\rangle$)  \label{l:get-help}
...
And it can address the imbalance  by \emph{sorting} the chunk.
...
The set of chunks to engage is determined by the rebalance policy.
...
It checks whether $C$ is immutable, and if so, helps the rebalance process that makes it immutable
...
Note that
put calls \code{checkRebalance} before incrementing \code{kCounter} and \code{vCounter} in order to avoid filling up infant chunks.
...
\codesize
	\begin{algorithmic}[1]{}
		\Procedure{checkRebalance}{$C$}
		\If{\codeF{$C$.status = infant}} 
			\If {$\neg$\codeF{rebalance($C$.parent, \codeF{key, val})}} \codeF{put(key, val)} \EndIf 
		\EndIf
...
\codesize
	...
	\Procedure{rebalance}{$C$, key, val}	
	\LineComment {\em 1. engage }
	\State \codeF{ro} $\leftarrow$ new rebalance object
	\State \codeF{ro.first} $\leftarrow$ \codeF{last} $\leftarrow C$
	\State CAS($C$.\codeF{ro}, $\bot$, \codeF{ro})
	\State \codeF{ro} $\leftarrow$ $C$.\codeF{ro}
	\State CAS(\codeF{ro.next}, $\bot, C$\codeF{.next})
	\While{ \codeF{ro.next} $\not=\bot$ }
		...
		\EndIf
	\EndWhile
	\State CAS(\codeF{ro.last}, $\bot$, \codeF{last})
	
	\LineComment {\em 2. freeze}
	\State $C$.\codeF{status} $\leftarrow$ \codeF{frozen}
			\ForAll {$c$ in $C$.\codeF{ppa}} 				
				\State \codeF{idx} $\leftarrow$ $c$.\codeF{idx} 
				\State CAS($c$, $\langle\bot$, \codeF{idx}$\rangle$, $\langle$\codeF{frozen, idx}$\rangle$)  
			\EndFor		
...
Since multiple threads may execute \code{rebalance($C$)} at the same time, they need to reach \emph{consensus} regarding
...
We exit the loop when \code{ro.next} is $\bot$, and then set \code{ro.last}  to  the last engaged chunk.
...
\paragraph{4. Creating new chunks.}

The next stage creates new chunks to replace the engaged ones. It traverses the list of engaged chunks from \code{ro.first} to \code{ro.last}.
...
the \code{next} pointer in \code{ro.last}. Second, the \code{next} pointer of the predecessor of \code{ro.first} needs to be set to the head of
the new chunks' list. In order to execute the two steps atomically, we do the following:
(1) \emph{mark} the \code{next} pointer in \code{ro.last} as immutable;
...
In the special case when the new list is empty (because no data is kept), step (3) CASes  the \code{next} pointer of the predecessor of \code{ro.first} to the  \code{next} pointer of \code{ro.last}.
...
Nevertheless, we do need to take into account races with old rebalance operations--- a thread that wakes up after being stalled must be prevented from indexing a chunk that had already been supplanted.
...
Finally, the status of the new chunks is changed to normal, and put operations may begin to update them.

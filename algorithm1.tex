...
The two procedures make use of thread-local variables $C_f$, $C_n$, and \codeF{last}. The latter tracks the last chunk engaged in rebalancing, whereas the first two hold pointers to the first and last new chunks, resp.
...
\codesize
	...
	\ForAll{\codeF{psa[t] = $\langle$ver, seq, from, to$\rangle$}}
		\If{\codeF{ro.first.minKey} $\le$ to $\wedge$
			...
			\If{ver=seq} add { $\langle$t, seq, from, to$\rangle$} to \codeF{toHelp} 
			\Else\  
...
%	\LongLineComment{we assume the \# of versions is much smaller than the chunk size}
	...
	\ForAll{chunks $C_o$ from \codeF{ro.first} to \codeF{last}}
		...
		\EndIf
		\ForAll{keys k in $C_o.ppa \cup C_o.k\  \cup $ \codeF{toPut} in sorted order}

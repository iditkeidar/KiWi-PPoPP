...
\codesize
	...
	\State \codeF{tmp} $\leftarrow$ new rebalance object, initially $\langle $C$, $C$.\codeF{next} \rangle$
...
%	\EndFor
	...
	\ForAll{\codeF{psa[t] = $\langle$ver, seq, from, to$\rangle$}}
		\If{\codeF{ro.first.minKey} $\le$ to $\wedge$
			...
			\Else\  
				minVersion $\leftarrow$ min(minVersion, ver)
			\EndIf
		\EndIf
	\EndFor				
	\If{toHelp $\not=\emptyset$}
...
%	\LongLineComment{we assume the \# of versions is much smaller than the chunk size}
	...
	\ForAll{chunk $C_o$ from \codeF{ro.first} to \codeF{last}}
		...
		\EndIf
		\ForAll{key k in $C_o.ppa \cup C_o.k\  \cup $ \codeF{toPut} in sorted order}
	...
	\State \codeF{last.mark} $\leftarrow$ true
	...
	\If{CAS(\codeF{pred.next+mark}, $C$+false, $C_f$+false)}
		\Comment{success}  
		\State normalize($C$); return true    
	\EndIf
	\If{\codeF{pred.next.parent} = $C$} 
		\Comment{someone else succeeded}
		\State normalize($C$); return false 

...
\label{l:put-prep}
		\If{\codeF{checkRebalance($C$, \codeF{key, val}})}
			\Comment put completed by rebalance    \label{l:put-rebalance0}
			\State return
		\EndIf
...
\codesize
	...
	\LineComment we assume the number of versions is much smaller than the chunk size
	\State $C_f \leftarrow C_n \leftarrow$ new chunk  
		\State \hspace{10mm} with \codeF{minKey}=$C$.\codeF{minKey}, \codeF{parent}=$C$, \codeF{status = infant} 
	\ForAll{chunks $C_o$ from \codeF{ro.first} to \codeF{ro.last}}
	...
	\State \codeF{ro.last.immutable} $\leftarrow$ true
	\State $C_n$.\codeF{next} $\leftarrow$ \codeF{ro.last.next}
	...
	\ForAll{chunks $c$ from $C_f$ to $C_n$}
		...
		\EndIf
	\EndFor		
	\ForAll{chunks $c$ from \codeF{ro.first} to \codeF{ro.last}}

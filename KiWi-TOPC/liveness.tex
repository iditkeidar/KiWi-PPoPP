...
\end{lemma}
\begin{proof}
By Proposition~\ref{prop:locate}, \code{locate} function is wait-free. 
...
Beyond \code{rebalance} calls and recursive calls to \code{put} when it fails, \code{put} executes two loops --  one in
the \code{locate} function and one in phase 3 (lines~\ref{l:put-ll}--\ref{l:put-ll-end}).  
...
We first argue that the first four stages of the \code{rebalance} function also complete within a finite number of steps. 
...
The first loop in stage 5 terminates once the CAS -- either of this thread or of another thread -- succeeds to mark \code{last.next} as immutable. 
...
%this means that another \code{rebalance} has succeeded. 

% \Idit{If someone else marked the next pointer and failed, then it set it to the last.next, because }  

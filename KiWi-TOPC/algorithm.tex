...
\label{l:put-prep}
		...
		\EndIf
		\State \codeF{j} $\leftarrow$ F\&A($C$.\codeF{vCounter, val.size}) \Comment{allocate place for value} \label{l:put-cas-j}
...
\Procedure{locate}{key} \label{l:locate-start}
	...
	\State traverse linked list from $C_0$ until the last chunk $C$ s.t.\ $C$.\code{next.minKey}  < key
	\State return $C$
...
by increasing $C$'s array  counters to the next available indices \code{i} and \code{j} for \code{k} and \code{v}, resp.
...
The \code{rebalance} procedure takes the put's key and value as parameters, and attempts to piggyback the put on the reblanace, i.e., insert the key and value to the newly created chunk. In case it fails, it returns false, in which case the put is restarted. 
...
%	\LongLineComment{we assume the \# of versions is much smaller than the chunk size}
	...
	\Do \Comment{set the next pointer and mark bit of the last old chunk to $C_n$'s next pointer and true} \label{l:mark}
		\State $C_n$.\codeF{next} $\leftarrow$ \codeF{last.next}
	\doWhile{$\neg$CAS(\codeF{last.next+mark}, $C_n$.\codeF{next}+false, $C_n$.\codeF{next}+true)} \label{l:mark-end}
	\Do 
...
The normalize procedure needs to know last, $C_n$, and $C_f$ in order to delete old chunks from the index and insert new ones. These may be stored in \code{ro} by reblance for this purpose, or may be discovered by traversals. 

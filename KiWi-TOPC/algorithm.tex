...
Pseudocode for these operations is given in Algorithm~\ref{alg:rebalance}.
The two procedures make use of thread-local variables $C_f$, $C_n$, and \codeF{last}. The latter tracks the last chunk engaged in rebalancing, whereas the first two hold pointers to the first and last new chunks, respectively.
...
\codesize
	...
	\State \codeF{tmp} $\leftarrow$ new rebalance object, with \codeF{first}=$C$, \codeF{next}=$C$.\codeF{next}
	\State CAS($C$.\codeF{ro}, $\bot$, \codeF{tmp})
	...
	\While{ \codeF{ro.next} $\not=\bot$ }
		...
		\EndIf
	\EndWhile
	\LineComment {search for the last concurrently engaged chunk}
	\While{\codeF{last.next.ro = ro}}
...
%	\EndFor
	...
	\ForAll{\codeF{psa[t] = $\langle$ver, seq, from, to$\rangle$}}
		\If{\codeF{ro.first.minKey} $\le$ to $\wedge$
			\State {\codeF{last.next.minKey} $>$ from} }
			\If{ver=?} add { $\langle$t, seq, from, to$\rangle$} to \codeF{toHelp} 
			\Else\  
...
%	\LongLineComment{we assume the \# of versions is much smaller than the chunk size}
	...
	\Do 
		...
		\If{CAS(\codeF{pred.next+mark}, $C$+false, $C_f$+false)}
			\Comment{success}  
			\State normalize($C$)
			\State return true    
		\EndIf
		\If{\codeF{pred.next.parent} = $C$} 
			...
			\State return false 
		\EndIf 
		\State rebalance(pred, $\bot, \bot$) \Comment{insertion failed, help predecessor}
...
(Algorithm~\ref{alg:ops}, line~\ref{l:put-frozen}). Otherwise, the put has already assigned its version, and
...
Note that scan's CAS (Algorithm~\ref{alg:ops}, line~\ref{l:scan-FiCAS})  might fail  in case it is helped, but
...
Keys are added, in sorted order, to a new chunk $C_n$ until it is roughly half full, at which point a new chunk $C'$ is created
and $C_n.\code{next}$ is set to $C'$. (In case the last chunk is too sparse, for example, only a quarter full, it is discarded and its keys are moved to the  penultimate chunk).

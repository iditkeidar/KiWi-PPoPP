...
and has also been used  in the context of transactional memory~\cite{mv-stm-chapter}. But in contrast to standard multi-versioning, \kiwi\ does not create a new version for each update, and leaves version number management to scans rather than updates.
...
However,~\cite{Petrank2013} supports only one scan at a time, and~\cite{wttm2016}'s throughput is lower than \kary's in low contention scenarios. 
